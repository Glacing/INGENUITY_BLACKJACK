...
gestionnaire_general = Gestionnaire()
...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, Gestionnaire)
...
# services
def Lancerpartie_callback(sender_agent_name, sender_agent_uuid, service_name, tuple_args, token, my_data):
    try:
        gestionnaire_general.Lancerpartie(sender_agent_name, sender_agent_uuid)
    except:
        print(traceback.format_exc())


def Donnermain_callback(sender_agent_name, sender_agent_uuid, service_name, tuple_args, token, my_data):
    try:
       carte =  gestionnaire_general.Donnermain(sender_agent_name,sender_agent_uuid)
       igs.output_set_string("whiteboardOutput",carte)
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    igs.service_init("lancerPartie", Lancerpartie_callback, agent)
    igs.service_init("donnerMain", Donnermain_callback, agent)
